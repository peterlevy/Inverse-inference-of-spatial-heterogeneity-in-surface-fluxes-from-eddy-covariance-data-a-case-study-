...
where p(s,t) is the footprint function for location s at time t; this function integrates to one over the area (\(\int_{footprint(t)} p(s,t) d s = 1)\). The footprint has irregular shape and varies over time. For landscape anaHowever, if we define a rectangular grid that extends well beyond the footprint, then equation \ref{eq:EC} can be approximated as follows:
