...
where p(s,t) is the footprint function for location s at time t. The footprint function integrates to one over the area (\\int_{footprint(t)} p(s,t) d s = 1). As equation \ref{eq:EC} shows, eddy covariance (EC) does not measure the flux over a fixed rectangular gridded landscape: each EC-measurement represents the non-rectangular footprint area of its measurement period. However, if we define a rectangular grid that extends well beyond the footprint, then equation \ref{eq:EC} can be approximated as follows:
