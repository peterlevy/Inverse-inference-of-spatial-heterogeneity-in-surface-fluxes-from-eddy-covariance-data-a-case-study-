...
\begin{align}   \label{eq:EC}
  F_{EC}(t) = \int_{footprint(t)} F_{s,t} p(s,t) d s,
\end{align}

where p(s,t) is the footprint function for location s at time t. The footprint function which integrates to one over the locations s in the footprint area (\int_{footprint(t)} p(s,t) d s = 1). As the equation shows, eddy covariance (EC) does not measure the flux over a fixed rectangular gridded landscape. Each EC-measurement represents the non-rectangular footprint area of its measurement period. The unequal contributions from different locations in the footprint area are represented by the continuous footprint function p(s), which integrates to unity. So the EC-measurements \(F_{EC}\) are given by:
