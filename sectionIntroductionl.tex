...
where p(s,t) is the footprint function for location s at time t. The footprint function integrates to one over the area (\(\int_{footprint(t)} p(s,t) d s = 1)\). However, if we define a rectangular grid that extends well beyond the footprint, then equation \ref{eq:EC} can be approximated as follows:
