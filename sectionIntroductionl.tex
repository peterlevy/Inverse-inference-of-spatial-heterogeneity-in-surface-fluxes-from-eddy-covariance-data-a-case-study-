...
In contrast to the above equation, eddy covariance (EC) does not measure the flux over a rectangular gridded landscape. Each EC-measurement represents the non-rectangular footprint area that applied . The unequal contributions from different locations in the footprint area are represented by the continuous footprint function p(s), which integrates to unity. So the EC-measurements \(F_{EC}\) are given by:
