...
where \(F_{xyt}\) is the flux in cell (x,y) at time t, which contributes to measured flux with weight \(W_{xyt}\). The weights
\(\mathbf{W} = \{W_{xy}\}\) that sum to unity at each time interval. We
